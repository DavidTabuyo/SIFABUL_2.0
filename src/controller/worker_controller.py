...
class WorkerController(QMainWindow):
    ...
    def update_checks(self):
        ...
        checks = WorkerDao.get_today_checks(self.worker.worker_id, get_current_time()[1])
        self.view.addChecks(checks)
        
    
    def update_notifications(self):
        self.view.clear_layout(self.view.notifications_layout)
        notifications=[notification for notification in NotificationWorkerDao.get_notifications_by_worker(self.worker.worker_id) if not notification.seen]
        if notifications:
    ...
    def check(self):
        ...
        CheckDao.add_new_check(self.worker.worker_id, date, time, is_new_check_entry)
    ...
    def delete_btn_clicked(self):
        NotificationWorkerDao.update_notification_status(self.worker.worker_id, get_current_time()[3])
        self.view.clear_layout(self.view.notifications_layout)
        self.view.show_label()
